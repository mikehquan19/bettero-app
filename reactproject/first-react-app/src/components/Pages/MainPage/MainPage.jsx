...
import TransactionForm from '../../Forms/AddForms/TransactionForm/TransactionForm.jsx';
...
const MainPage = ({ navbarWidth, titleHeight }) => {
	...
	const fetchData = () => {
		Promise.all([
			...
			.then((responseList) => {
				setLatestTransList(responseList[0].data);
				// set the financial info
				const summaryData = responseList[1].data;
				setFinancialInfo({
				...
				setAccountList(responseList[2].data);
				console.log("get summary data request status: " + responseList.status); 
			})
	...
	const handleCategoryClick = (e, argChartRef) => {
		...
		if (selectedElement) {
			...
			expenseappClient.get(`/transactions/category/${newCategory}`)
				...
				.catch((error) => handleError(error));
			console.log(tableCategoryObject.categoryTransList);
		} else {
	...
	// the callback passed to categorized transaction tables
	// take the searched name and modify the transaction list of be rendered
	const handleSearchClick = (searchTransName, tableType) => {
		if (searchTransName !== "") {
			// modify the rendered list of transactions based on type of table 
			if (tableType === "category") {
				setTableCategoryObject((previousData) => ({
					...previousData,
					categoryTransList: tableCategoryObject.categoryTransList.filter(transaction =>
						transaction.description === searchTransName
					)
				}))
				console.log(tableCategoryObject);
			}
			else if (tableType === "latest") {
				const filteredTransList = latestTransList.filter(transaction =>
					transaction.description === searchTransName
				)
				setLatestTransList(filteredTransList);
			}
		}
		else {
			if (tableType === "category") {
				expenseappClient.get(`/transactions/category/${tableCategoryObject.category}`)
					.then((response) => setTableCategoryObject((previousData) => ({
						...previousData,
						categoryTransList: response.data,
					})))
					.catch((error) => handleError(error));
				...
				expenseappClient.get("/transactions")
					.then((response) => setLatestTransList(response.data))
					.catch((error) => handleError(error));
			}
		}
	}
	...
	const debitAccountList = accountList.filter(account => 
		account.account_type === "Debit"
	);

	const creditAccountList = accountList.filter(account => 
	...
	return (
		<div style={{
			...
			<div className="charts" >
				<div className="main-charts" style={{ textAlign: "center", }}>
					...
					<div className="chart-wrapper">
						...
						<div className="composition-chart-wrapper">
							<CompositionChart compositionObject={expenseComposition} handleCategoryClick={handleCategoryClick} />
						</div>
					</div>
					<p>Click on the each category in the pie chart to show the user's list of transactions </p>
				</div>
				{tablePresent && (
					<>
						<TransactionTable
							listName={tableCategoryObject.category !== "Income" ?
								...
								tableCategoryObject.category + " transactions this month"}
			...
			<CardFrame cardType="debit"
				numCards={debitAccountList.length}>
				{/* The list of debit accounts */}
				{debitAccountList.length === 0 && (
					<p style={{						
					 	fontWeight: "bold", 
						margin: "0 auto",
					}}>There are no debit accounts</p>
				)}
				{debitAccountList.map(debitAccount =>
					<Card
						key={debitAccount.id} // key to distinguish debit cards 
			...
			{debitFormPresent &&
				(<AccountForm 
					...
					handleHideForm={handleDebitPresentClick} />)}
			<br />

			<CardFrame cardType="credit"
				numCards={creditAccountList.length}>
				{/* the list of credit accounts */}
				{creditAccountList.length === 0 && (
					<p style={{						
					 	fontWeight: "bold", 
						margin: "0 auto",
					}}>There are no credit accounts</p>
				)}
				{creditAccountList.map(creditAccount =>
					<Card
						key={creditAccount.id} // key to distinguish credit cards
			...
			{creditFormPresent &&
				(<AccountForm 
					...
					handleHideForm={handleCreditPresentClick} />)}

			<TransactionTable
				listName="Latest transactions"
				transactionList={latestTransList}
				handleSearchClick={handleSearchClick}
				tableType="latest" />
			{!transFormPresent && (
				<div style={{
					...
					handleEvents={handleTransFormPresentClick} />
				</div>
			)}
