...
const BudgetPlanForm = ({
  ...
  const getAPIEndpoint = (argType, argIntervalType, argData) => {
    if (argType === "ADD") {
  ...
  const handleBudgetFormSubsmit = (data, e) => {
    ...
    if (sum !== 100) {
      ...
      const intervalType = existingData === null ? null : existingData.interval_type;
      getAPIEndpoint(type, intervalType, submittedData)
