...
func GetAccounts(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()

		accounts, err := accService.ListAccounts(ctx, UserID)
		if err != nil {
...
func GetAccount(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()
		...
		account, err := accService.GetAccount(ctx, int64(id))
		if err != nil {
...
func PostAccounts(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()
		...
		newAccount, err := accService.CreateAccount(ctx, UserID, body)
		if err != nil {
...
func PutAccount(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()
		...
		updatedAccount, err := accService.UpdateAccount(ctx, int64(id), body)
		if err != nil {
			if errors.Is(err, models.ErrInvalidAccountBody) {
				respondError(c, http.StatusBadRequest, err)
...
func DeleteAccount(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()
		...
		if err = accService.DeleteAccount(ctx, int64(id)); err != nil {
			if errors.Is(err, models.ErrNotFound) {
				respondError(c, http.StatusNotFound, err)
			} else {
				respondError(c, http.StatusInternalServerError, err)
...
func GetAccountTransactions(accService *services.AccountService) gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(c.Request.Context(), 10*time.Second)
		defer cancel()
		...
		total, transactions, err := accService.ListAccountTransactions(ctx, int64(id), filter, int64(offset))
		if err != nil {
			respondError(c, http.StatusInternalServerError, err)
			return
		}
