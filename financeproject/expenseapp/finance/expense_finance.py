...
def total_income(user: User, first_date: date=None, last_date: date=None) -> Decimal: 
    ...
    income_list = Transaction.objects.filter(
        user=user, category=INCOME, 
        occur_date__gte=proc_first_date, occur_date__lte=proc_last_date)

    # Compute the total income 
    total_income = Decimal(income_list.aggregate(total=Sum("amount", default=0))["total"])
    return total_income 


def daily_expense(user: User, first_date: date=None, last_date: date=None) -> Dict: 
    """ 
    ...
    while current_date <= proc_last_date: 
        ...
        total_expense = Decimal(curr_expense_list.aggregate(total=Sum("amount", default=0))["total"])
...
def expense_composition_percentage(
    object: Union[User, Account], first_date: date=None, last_date: date=None
) -> Dict: 
    """
    ...
    proc_first_date, proc_last_date = get_curr_dates("month", first_date, last_date)
    expense_dict = category_expense_dict(object, proc_first_date, proc_last_date)
    
    # Dictionary mapping the expense's category to the percentage of expense
    percent_dict = {
        category : Decimal(0.0) for category in list(CATEGORY_DICT.keys()) if category != INCOME
    }
    ...
    if expense_dict[TOTAL] != 0:  
        ...
        for category in list(percent_dict.keys()):  
            percent_dict[category] = expense_dict[category] / expense_dict["Total"]
            percent_dict[category] = round(percent_dict[category] * 100, 2)

    return percent_dict


def expense_change_percentage(
    object: Union[User, Account], period_type: str=MONTH, 
    ...
    curr_expense_dict = category_expense_dict(object, curr_date1, curr_date2)
    prev_expense_dict = category_expense_dict(object, prev_date1, prev_date2)
    ...
    for category in list(percent_dict.keys()): 
        if prev_expense_dict[category] != 0: 
            ...
            percent_dict[category] = curr_expense_dict[category] - prev_expense_dict[category]
            percent_dict[category] = round(percent_dict[category] / prev_expense_dict[category] * 100, 2)
        else: 
            # If no expenses made during previous month, expenses increase 100% 
            if curr_expense_dict[category] != Decimal(0): 
                percent_dict[category] = Decimal(100.00)
            else: 
                percent_dict[category] = Decimal(0.00)
    return percent_dict
...
def interval_total_expense(user: User) -> Dict: 
    ...
    for interval_type in [MONTH, BI_WEEK, WEEK]: 
        # get the data of 5 latest periods 
        latest_periods_dict[interval_type] = latest_periods(interval_type, 5)
